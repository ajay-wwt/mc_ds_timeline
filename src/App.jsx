...
export default function MCDSTimeline() {
  ...
  const [newEventLabel, setNewEventLabel] = useState(""); const [newEventOtherTitle, setNewEventOtherTitle] = useState(""); const [addEventError, setAddEventError] = useState("");
  ...
  const openAddEvent = (person) => { setAddTarget(person); setNewEventType("joined_team"); setNewEventYear(2026); setNewEventMonth(1); setNewEventLabel(""); setNewEventOtherTitle(""); setAddEventError(""); setShowAddEventModal(true); };
  const submitEvent = () => { if (!addTarget) return; if (newEventType === "joined_team" && addTarget.events.some(e => e.type === "joined_team")) { setAddEventError("This person already has a \"Joined Team\" event."); return; } setAddEventError(""); const date = `${newEventYear}-${String(newEventMonth).padStart(2, "0")}`; const label = newEventType === "other" ? (newEventOtherTitle || "Other event") : (newEventLabel || allTypes[newEventType]?.label || "Event"); updatePeople(prev => prev.map(p => p.id === addTarget.id ? { ...p, events: [...p.events, { id: nextEid(), date, type: newEventType, label, photos: [] }].sort((a, b) => a.date.localeCompare(b.date)) } : p)); setShowAddEventModal(false); };
